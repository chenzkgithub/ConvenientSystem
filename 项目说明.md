...
│       └── yunhan/                # 云瀚考勤模块
├── db/
│   ├── init.sql                   # 数据库初始化脚本（PC 端/两端共用对象，幂等，唯一维护入口）
│   └── init-app.sql               # 数据库初始化脚本（手机端专属对象，幂等，须在 init.sql 之后执行）
├── 启动后端开发服务.cmd            # 启动后端开发服务（端口 51943）
├── 启动前端开发服务.cmd            # 启动前端热更新服务（端口 5173，代理到 51943）
├── 一键发布打包.cmd                # 发布打包（API → api/，桌面端 → exe/）
├── 启动已发布程序.cmd              # 启动已发布的程序
├── 构建安装包.cmd                  # 构建桌面端安装包（含版本号自增 + Inno Setup）
├── 更新已安装前端.cmd              # 给已安装程序热更新前端 wwwroot
├── 阿里云部署指南.md               # 阿里云 Windows + SQL Server 部署指南
└── Linux云服务器部署教程.md         # Linux + SQLite 云服务器部署教程
```
