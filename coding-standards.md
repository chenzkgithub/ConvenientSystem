...
5. **新增页面必须注册视图管理**：每新增一个前端页面，必须按以下顺序完成注册：① 在 `SysView` 表注册视图（分配 Id、Name、Title、Component、RoutePath）；② 在 `SysViewPermission` 表维护该视图的动作权限点（如增删改查等操作按钮）；③ 将视图权限分配给相应角色（`SysRoleViewPerm`）；④ 最后在 `SysMenu` 菜单表中关联该视图。禁止跳过视图注册直接添加菜单。同步更新 init.sql 种子数据。
...
**新增页面必须注册视图管理并维护动作权限点**
- 每新增一个前端页面，必须按顺序完成：① SysView 注册视图；② SysViewPermission 维护动作权限点（页面访问 + 操作按钮）；③ SysRoleViewPerm 分配角色权限；④ SysMenu 关联菜单
- 禁止跳过视图注册直接添加菜单，同时更新 init.sql 种子数据
