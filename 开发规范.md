...
4. **SQL 变更自动执行**：修改 db/init.sql、db/init-app.sql 后（见 4.2 唯一入口规矩），自动在本地 LocalDB 顺序执行变更，不需用户额外确认。
5. **新增页面必须注册视图管理**：每新增一个前端页面，必须按以下顺序完成注册：① 在 `SysView` 表注册视图（分配 Id、Name、Title、Component、RoutePath）；② 在 `SysViewPermission` 表维护该视图的动作权限点（如增删改查等操作按钮）；③ 将视图权限分配给相应角色（`SysRoleViewPerm`）；④ 最后在 `SysMenu` 菜单表中关联该视图。禁止跳过视图注册直接添加菜单。同步更新 db/init.sql 种子数据。
...
在 db/init.sql 或 db/init-app.sql 中新建表或新增字段时，必须同时维护中文注释：表需写 `EXEC dbo.usp_AddTableComment` 语句，列需用 `EXEC dbo.usp_AddColumnComment` 或 `-- 中文说明` 形式注释。

### 4.2 DB 变更唯一入口：init.sql + init-app.sql，严禁另建迁移脚本

所有表结构变更、初始数据调整、老库升级补丁统一维护在 db/ 下两个初始化脚本中：
- PC 端/两端共用对象（含 Chat 会话/成员/消息/屏蔽/转发表及其老库补列）改 `db/init.sql`；
- 手机端专属对象（如 SysUserAppPerm 权限白名单、好友体系两表、存量互加）改 `db/init-app.sql`（须在 init.sql 之后执行，依赖其中的库、ChatConversation 表与注释存储过程）。

两文件均幂等可重复执行；部署链路打包时按 init.sql → init-app.sql 顺序拼接为单一 db/init.sql 随容器自动执行（见 .github/workflows/deploy.yml 与 docker/init-db.sh），老库随部署自动补齐。严禁按需求另建 migrate-*.sql 迁移脚本（不在部署包内，需人工执行极易遗漏）。
...
- 启动步骤：1) 初始化数据库 `sqllocaldb start MSSQLLocalDB` 后 `sqlcmd -S (localdb)\MSSQLLocalDB -E -i db\init.sql`；2) 安装前端依赖 `cd web && npm install`；3) 分别运行 `启动后端开发服务.cmd` 和 `启动前端开发服务.cmd`
- 默认账号：admin / admin

**构建与发布配置**
- `启动后端开发服务.cmd`：启动后端开发服务（端口 51943）
- `启动前端开发服务.cmd`：启动前端热更新服务（端口 5173，代理到 51943）
- `一键发布打包.cmd`：一键发布（前端构建 → API 打包到 api/ → 桌面端打包到 exe/）
- `启动已发布程序.cmd`：启动已发布的程序
...
- 教训：多会话场景下错误处理里必须比对当前 token 与请求 token，过滤过期响应
