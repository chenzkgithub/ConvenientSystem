...
public sealed class PipelineService
{
    ...
    private async Task RunAsync(RunningPipeline run, PipelineDefinition definition)
    {
        ...
        AppendLog(run, $"════════ 流水线【{definition.Name}】开始 ════════");

        var failed = false;
        var cancelled = false;
        string? lastBuildOutputDir = null;
        string? lastProjectDir = null;
        // 最近一次构建阶段的产物类型：部署阶段自动跟随（服务名/远程目录默认值按它推导），
        // 杜绝“构建 Web 后部署却按 DotNet 默认值切目录”这类错配
        UniversalBuildType? lastBuildType = null;
        ...
        for (var i = 0; i < definition.Stages.Count; i++)
        {
            ...
            try
            {
                ...
                {
                    var dto = await RunBuildStageAsync(run, rec, definition, stage, lastBuildOutputDir, lastProjectDir, ct);
                    lastBuildOutputDir = dto.OutputDir;
                    lastBuildType = stage.BuildType;
                    if (!string.IsNullOrWhiteSpace(stage.ProjectDir)) lastProjectDir = stage.ProjectDir;
                ...
                {
                    var (fileCount, batchCount, affected) = await RunSqlStageAsync(run, rec, stage, lastBuildOutputDir, ct);
                    rec.Message = $"{stage.DbType} 执行 {fileCount} 文件/{batchCount} 批次，影响 {affected} 行";
                }
                else
                {
                    await RunDeployStageAsync(run, rec, definition, stage, lastBuildOutputDir, lastBuildType, ct);
                    rec.Message = $"部署至 {stage.Host}";
                }
                rec.Status = PipelineStageRunStatus.Success;
                rec.Progress = 100;
                rec.StepText = string.Empty;
            ...
            catch (OperationCanceledException)
            {
                ...
                rec.Message = "已取消";
                // 保留 StepText：取消时进行到的步骤（如 [3/7] 上传中）在卡片上有诊断价值
            ...
            catch (Exception ex)
            {
                ...
                // 保留 StepText：失败瞬间的步骤文本（如 执行 init.sql（2/5））供卡片展示
    ...
    /// <summary>执行构建阶段：启动构建任务并轮询到终态，返回终态 DTO（供取产物目录）；轮询期间回写阶段进度。</summary>
    private async Task<UniversalBuildJobDto> RunBuildStageAsync(
        RunningPipeline run, PipelineRunStageRecord rec, PipelineDefinition definition, PipelineStage stage, string? lastBuildOutputDir, string? lastProjectDir, CancellationToken ct)
    {
        var isInstaller = stage.BuildType == UniversalBuildType.Installer;
        var projectDir = !string.IsNullOrWhiteSpace(stage.ProjectDir)
            ? stage.ProjectDir
            : (isInstaller ? lastProjectDir : lastBuildOutputDir) ?? string.Empty;

        // Installer 阶段需要 .iss 脚本位于项目目录；脚本内通常引用 ../exe/xxx.exe 等固定相对路径。
        // 如果上一步产物目录里有同名 exe，自动复制到安装脚本预期的相对目录，实现"先构建桌面端，再打包安装包"。
        if (isInstaller && !string.IsNullOrWhiteSpace(projectDir) && !string.IsNullOrWhiteSpace(lastBuildOutputDir))
        {
            TrySyncInstallerExeSource(run, projectDir, lastBuildOutputDir);
        }
        ...
        while (true)
        {
            ...
            // 回写阶段进度：与通用构建页卡片同源的插值进度；排队时提示排队位置
            rec.Progress = current.Progress;
            rec.StepText = current.QueuePosition is { } queuePosition ? $"排队中 #{queuePosition}" : string.Empty;
    ...
    /// <summary>
    /// 安装包阶段执行前，把上一步产物目录中的 exe 同步到安装脚本预期的 ../exe/ 相对目录。
    /// 这样流水线"先构建桌面端 → 再 Inno Setup 打包"时，用户无需手动把产物放到 exe/ 目录。
    /// </summary>
    private static void TrySyncInstallerExeSource(RunningPipeline run, string projectDir, string sourceOutputDir)
    {
        try
        {
            var expectedExeDir = Path.GetFullPath(Path.Combine(projectDir, "..", "exe"));
            if (!Directory.Exists(expectedExeDir)) Directory.CreateDirectory(expectedExeDir);

            var sourceFiles = Directory.EnumerateFiles(sourceOutputDir, "*.exe", SearchOption.TopDirectoryOnly).ToList();
            if (sourceFiles.Count == 0) return;

            foreach (var sourceFile in sourceFiles)
            {
                var destFile = Path.Combine(expectedExeDir, Path.GetFileName(sourceFile));
                if (File.Exists(destFile)) continue; // 已存在则不覆盖，尊重用户预置
                File.Copy(sourceFile, destFile, overwrite: false);
                AppendLog(run, $">> 已同步安装包源文件：{sourceFile} → {destFile}");
            }
        }
        catch (Exception ex)
        {
            AppendLog(run, $">> ⚠ 同步安装包源文件失败：{ex.Message}，将继续尝试编译安装脚本");
        }
    }

    /// <summary>执行部署阶段：组装 DeployRequest 并轮询到终态；轮询期间回写阶段进度与步骤文本。</summary>
    private async Task RunDeployStageAsync(
        RunningPipeline run, PipelineRunStageRecord rec, PipelineDefinition definition, PipelineStage stage,
        string? lastBuildOutputDir, UniversalBuildType? lastBuildType, CancellationToken ct)
    {
        ...
        var jobId = _deployService.StartDeploy(new DeployRequest
        {
            ...
            // 前面有构建阶段时部署类型自动跟随构建产物（与产物目录同一规则）；
            // 纯部署流水线才用阶段里手选的 DeployBuildType 兜底
            BuildType = lastBuildType ?? stage.DeployBuildType,
        ...
        while (true)
        {
            ...
            // 回写阶段进度与步骤文本（步骤文本与通用构建页部署卡片的 [n/m] 标题同格式）
            rec.Progress = job.Progress;
            rec.StepText = string.IsNullOrWhiteSpace(job.StepTitle)
                ? string.Empty
                : $"[{job.CurrentStep}/{job.TotalSteps}] {job.StepTitle}";
    ...
    private async Task<(int FileCount, int BatchCount, long Affected)> RunSqlStageAsync(
        RunningPipeline run, PipelineRunStageRecord rec, PipelineStage stage, string? lastBuildOutputDir, CancellationToken ct)
    {
        ...
        for (var i = 0; i < files.Count; i++)
        {
            ...
            // 回写阶段进度：按文件序号推进（单文件执行内部无法感知进度）
            rec.StepText = $"执行 {fileName}（{i + 1}/{files.Count}）";
            rec.Progress = i * 100 / files.Count;
    ...
    private static void AppendLog(RunningPipeline run, string line)
    {
        lock (run.Log)
        {
            // 流水线状态行统一带时间戳前缀（事后回看可判断各环节起止与卡点）；空行原样，子任务增量日志走 AppendLogDelta 不加前缀
            run.Log.AppendLine(line.Length == 0 ? line : $"[{DateTime.Now:HH:mm:ss}] {line}");
        }
    }
