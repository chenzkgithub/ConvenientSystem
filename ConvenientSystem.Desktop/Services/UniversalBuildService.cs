...
public sealed class UniversalBuildService
{
    ...
    private async Task RunBuildAsync(UniversalBuildJob job)
    {
        ...
        try
        {
            ...
            // Installer 类型需要先 dotnet publish 产物到 ../exe，再运行 iscc。
            // 否则安装包读取的是旧 exe 的版本，导致安装包版本与代码版本不一致。
            if (job.Type == UniversalBuildType.Installer)
            {
                var csproj = FindProjectFile(workingDir, "*.csproj");
                var exeDir = Path.GetFullPath(Path.Combine(workingDir, "..", "exe"));
                Directory.CreateDirectory(exeDir);
                var publishArgs = $"publish \"{csproj}\" -c Release -r win-x64 --self-contained true -p:PublishSingleFile=true -p:SkipVueBuild=true -o \"{exeDir}\"";
                AppendLog(job, ">> 安装包类型：先编译桌面端产物到 ../exe（自包含单文件）");
                var publishExit = await RunProcessAsync(job, "dotnet", publishArgs, workingDir);
                if (publishExit != 0)
                {
                    job.ExitCode = publishExit;
                    job.Status = UniversalBuildStatus.Failed;
                    AppendLog(job, ">> dotnet publish 失败，安装包打包中止");
                    return;
                }
            }

            var exitCode = await RunProcessAsync(job, fileName, arguments, workingDir);
            if (exitCode == -1 && job.Status == UniversalBuildStatus.Cancelled)
                return;

            job.ExitCode = exitCode;
            AppendLog(job, string.Empty);
            AppendLog(job, $">> 任务结束，退出码：{exitCode}");


            // 构建成功后把产物归集到输出目录（Web/Node 与 Java 命令不接收输出目录，产物默认落在项目内）。
            // 状态在归集/统计全部完成后才置 Success：前端轮询见到 Success 时产物统计必为最终值
            if (exitCode == 0)
            {
    ...
    /// <summary>启动外部进程并等待退出；取消时设置 job.Status = Cancelled 并返回 -1。</summary>
    private async Task<int> RunProcessAsync(UniversalBuildJob job, string fileName, string arguments, string workingDir)
    {
        var resolved = LocalBuildService.FindInPath(fileName);
        if (string.IsNullOrEmpty(resolved))
            resolved = GetCandidatePath(fileName) ?? fileName;

        // iscc 在 PATH 与常见安装路径均未找到时，给出明确安装提示
        if (!File.Exists(resolved) && fileName.Equals("iscc", StringComparison.OrdinalIgnoreCase))
        {
            throw new InvalidOperationException(
                "未找到 Inno Setup 编译器（iscc）。请安装 Inno Setup 6（https://jrsoftware.org/isdl.php）后重试。");
        }
        ...
        try
        {
            await process.WaitForExitAsync(job.Cts?.Token ?? CancellationToken.None);
        }
    ...
    /// <summary>
    /// 把本地文件夹打包成 zip（独立打压缩包入口，与构建流程解耦）：
    /// targetZip 留空时落在源文件夹同级目录（不能放源文件夹自身内，否则会被打进包），
    /// 时间戳命名不覆盖旧包。返回 (zip 完整路径, 大小字节)。
    /// </summary>
    public static (string Path, long Size) PackFolderToZip(string sourceDir, string? targetZip)
    {
        sourceDir = sourceDir.Trim().TrimEnd('\\', '/');
        if (string.IsNullOrEmpty(sourceDir))
            throw new ArgumentException("源文件夹不能为空");
        if (!Directory.Exists(sourceDir))
            throw new ArgumentException($"源文件夹不存在：{sourceDir}");
        if (!Directory.EnumerateFileSystemEntries(sourceDir).Any())
            throw new ArgumentException($"源文件夹为空：{sourceDir}");

        var dirName = Path.GetFileName(sourceDir);
        if (string.IsNullOrEmpty(dirName)) dirName = "archive";
        var zipPath = !string.IsNullOrWhiteSpace(targetZip)
            ? targetZip.Trim()
            : Path.Combine(
                Path.GetDirectoryName(sourceDir) ?? AppContext.BaseDirectory,
                $"{dirName}_{DateTime.Now:yyyyMMdd-HHmmss}.zip");

        // 目标已存在时先删（手填路径的明确覆盖语义；自动命名的带时间戳不会撞）
        if (File.Exists(zipPath)) File.Delete(zipPath);
        ZipFile.CreateFromDirectory(sourceDir, zipPath, CompressionLevel.Optimal, true);
        return (zipPath, new FileInfo(zipPath).Length);
    }
