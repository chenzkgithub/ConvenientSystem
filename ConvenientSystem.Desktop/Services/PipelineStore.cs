...
public sealed class PipelineStage
{
    ...
    /// <summary>输出目录是否由用户手动指定；false = 跟随阶段名/构建类型自动生成。</summary>
    public bool OutputDirCustom { get; set; }
    ...
    /// <summary>部署产物类型：前序有构建阶段时运行时自动跟随构建类型（与产物目录同一跟随规则），
    /// 此字段仅纯部署流水线（前面无构建阶段）时生效，用于默认服务名/远程目录推断。</summary>
...
public sealed class PipelineRunStageRecord
{
    ...
    /// <summary>阶段进度（0-100）：构建=通用构建插值进度、部署=步骤刻度进度、SQL=按文件序号；成功结束置 100。</summary>
    public int Progress { get; set; }
    /// <summary>当前步骤文本（运行中显示，如"[3/7] SFTP 上传到服务器""执行 init.sql（2/5）""排队中 #2"）；阶段结束清空。</summary>
