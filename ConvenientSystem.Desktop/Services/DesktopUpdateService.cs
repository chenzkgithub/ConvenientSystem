...
internal static class DesktopUpdateService
{
    ...
    /// <summary>
    /// 启动下载好的安装程序并返回进程对象，由调用方确认进程已启动后退出当前进程。
    /// </summary>
    public static Process LaunchInstaller(string setupPath)
    {
        if (!File.Exists(setupPath))
            throw new FileNotFoundException("安装程序不存在", setupPath);

        // 不再使用 /SILENT：让用户能看到安装向导，避免"点击更新后程序直接消失"的困惑。
        // /CLOSEAPPLICATIONS  安装前关闭正在运行的目标程序
        // /RESTARTAPPLICATIONS 安装完成后重新启动目标程序
        // /LOG 写入安装日志，便于排查"点了更新但没反应"、UAC 拒绝等问题。
        var logDir = Path.Combine(AppContext.BaseDirectory, "logs");
        Directory.CreateDirectory(logDir);
        var logPath = Path.Combine(logDir, $"installer-update-{DateTime.Now:yyyyMMddHHmmss}.log");
        var psi = new ProcessStartInfo(setupPath)
        {
            Arguments = $"/CLOSEAPPLICATIONS /RESTARTAPPLICATIONS /LOG=\"{logPath}\"",
        ...
        try
        {
            return Process.Start(psi) ?? throw new InvalidOperationException("无法启动安装程序");
        }
        catch (Exception ex)
        {
            throw new InvalidOperationException($"启动安装程序失败：{ex.Message}。可能是 UAC 授权被拒绝或安装包损坏。", ex);
        }
    }
