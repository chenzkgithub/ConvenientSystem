...
public sealed class MainForm : Form
{
    ...
    private string StartUrl => _webHost.BaseUrl;

    // 启动时直接显示主窗口
    private bool _allowVisible = true;

    // 共享的 WebView2 环境：内嵌浏览器和弹出窗口复用它以保持 Cookie / 登录态一致。
    ...
    // 系统帮助对话框（版本信息 + 手动检查更新）：单实例，重复打开时激活前置
    private readonly SystemHelpDialog _helpDialog;
    ...
    // 快速启动器工厂：按需创建启动器窗口
    private readonly QuickLauncherFactory _launcherFactory;

    private readonly LauncherStoreFactory _storeFactory;
    ...
    internal MainForm(
        ...
        LauncherStoreFactory storeFactory,
        QuickLauncherFactory launcherFactory,
        SystemHelpDialog helpDialog,
    ...
    private async void OnFormLoadAsync(object? sender, EventArgs e)
    {
        try
        {
            ...
            // 先尝试创建环境；若因用户数据目录损坏/不兼容失败，自动清理目录后重试一次。
            _env = await CreateWebView2EnvironmentAsync(userDataFolder, envOptions);
            ...
            // === 快速启动器 + 全局热键 ===
            _store = _storeFactory.Create();
            _launcher = _launcherFactory.Create(
                _store,
        ...
        catch (Exception ex)
        {
            ...
            var message = "初始化 WebView2 失败。\n\n" +
                $"当前线程 ApartmentState：{apartmentState}（WebView2 要求 STA）\n" +
                $"错误信息：{ex.Message}\n\n";

            if (apartmentState != ApartmentState.STA)
            {
                message +=
                    "检测到当前线程不是 STA（单线程单元），这通常由启动代码或某些 SDK 初始化导致。\n" +
                    "请尝试：1) 重启程序；2) 检查是否还有其他 WebView2/Edge 进程在运行并结束它们；3) 重新安装 WebView2 Runtime。\n\n";
            }

            message += "如果问题持续，请先安装 Microsoft Edge WebView2 运行时。\n" +
    ...
    /// <summary>
    /// 创建 WebView2 环境。若因用户数据目录损坏或不兼容失败，自动清理目录后重试一次。
    /// </summary>
        ...
        CoreWebView2EnvironmentOptions options)
    {
        ...
        {
            // 可能是用户数据目录损坏（如不同版本 SDK 写入导致不兼容），清理后重试。
            try { Directory.Delete(userDataFolder, recursive: true); }
            catch { /* 删除失败时忽略，继续尝试重建 */ }
    ...
    private async void OnNewWindowRequested(object? sender, CoreWebView2NewWindowRequestedEventArgs e)
    {
        ...
        try
        {
            var browser = _services.GetRequiredService<BrowserForm>();
    ...
    private void InitFloatingButton()
    {
        _floatingBtn = new FloatingButton(iconSize: 40)
        {
            ...
            SystemHelpAction = ShowSystemHelpDialog,
    ...
    /// <summary>添加两套菜单共用的固定项：系统帮助 / 关闭所有弹窗 / 重启 / 退出登录 / 退出程序。</summary>
    private void AddCommonMenuItems(ContextMenuStrip menu)
    {
        ...
        var helpItem = new ToolStripMenuItem("系统帮助");
        helpItem.Click += (_, _) => ShowSystemHelpDialog();
    ...
    /// <summary>
    /// 打开系统帮助对话框（版本信息 + 手动检查更新）。
    /// 单实例：已打开时激活前置，不重复弹窗。
    /// </summary>
    private void ShowSystemHelpDialog()
    {
        if (_helpDialog.Visible)
        {
    ...
    private async void OpenPageWindow(string page, string title, bool external = false)
    {
        ...
        if (!external && !page.StartsWith("http://", StringComparison.OrdinalIgnoreCase) && !page.StartsWith("https://", StringComparison.OrdinalIgnoreCase))
        {
            ...
            if (!isLoggedIn)
            {
                ...
                // 尝试让主窗口前端跳转到登录页（若已加载）
                try { _webView.CoreWebView2?.Navigate(StartUrl); } catch { /* 忽略 */ }
                return;
            }
        }
    ...
    private async void LogoutApp()
    {
        ...
        try { core.Navigate(StartUrl); } catch { /* 导航失败忽略 */ }
